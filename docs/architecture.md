...
Geometric Algebra and rendering them as interactive SVG. It currently ships six
algebras — **PGA(2,0,1)** (2D projective, default), **VGA(2,0,0)** (2D vectorial),
**R(0,1,0)**, **CGA Cl(3,1)** (`cga310`), **CCGA** (ℝ(5,3) conic conformal), and
**ACGA** (ℝ(4,2) axis-aligned conic conformal) — and is designed so a new algebra
is *one new directory* in `src/algebras/`.
...
attaches them to its `spec`. The `spec` interface (the `AlgebraSpec` consumed by
the graph factories) is summarised in the registry header `src/algebras/index.js`;
read any `src/algebras/<id>/index.js` for a concrete, complete spec.
...
3. **Classify & color.** `classifyMV(val).kind` drives both the panel vignette and
   the canvas color: the kind indexes the spec's `KIND_COLOR` map (kind→hex),
   applied in `useGraph.js`'s `colorMap` memo and mirrored by `ExpressionPanel`, so
   panel and canvas never disagree.
...
**Shared (one implementation, bound to a spec):** the four graph factories in
`src/graph/` (`parseExpression`, `evalMVArith`, `nodeTypes`, `evaluate`), the
`Canvas` renderer + drag layer, and `useGraph`. None of these contain literal blade
indices — anything index-specific is asked of the spec.

**Per-adapter (each `src/algebras/<id>/`):** `bladeIndex`/`bladeNames`, a
`parseBladeName` matcher (permuted-blade aware, e.g. `e21 = -e12`), `classifyMV`,
`getRenderPlan`, `KIND_COLOR` (kind→hex), `INITIAL_ITEMS` (the showcase), and the
optional point-embedding/parametric-drag hooks. PGA additionally keeps its heavy
math in `src/pga.js`, with `pga201/index.js` a thin spec wrapper over it.
