...
// verbatim copy of CCGA's; only the coefficient/dual extraction is (4,2)-specific.

import { A, GRADES, zeroMV, Iinv, gradeFlags, onlyGrade } from './algebra.js';

// IPNS grade-1 orthogonal coefficients → implicit axis-aligned conic. The cross
// term C is structurally zero in ℝ(4,2). Slots: [1]=D [2]=E [3]=-2A+F/4 [4]=-2B+F/4
// [5]=-2A-F/4 [6]=-2B-F/4  ⇒
export function coeffsFromGrade1(s) {
  const c1 = s[1]||0, c2 = s[2]||0, c3 = s[3]||0, c4 = s[4]||0, c5 = s[5]||0, c6 = s[6]||0;
  return {
    A: -(c3 + c5) / 4, B: -(c4 + c6) / 4, C: 0,
    D: c1, E: c2, F: (c3 - c5) + (c4 - c6),
  };
}

// Precomputed grade-5 → grade-1 dual map. Mul(C5, Iinv) sends a pure grade-5 blade
// to grade-1, so it is a small linear map on the grade-5 / grade-1 slots — building
// it once lets a grade-5 conic dualize with a tiny product.
const GRADE5_IDX = GRADES.map((g, i) => (g === 5 ? i : -1)).filter((i) => i >= 0);
const DUAL5 = (() => {
  const m = Array.from({ length: 7 }, () => new Array(GRADE5_IDX.length).fill(0));
  GRADE5_IDX.forEach((idx, c) => {
    const unit = zeroMV(); unit[idx] = 1;
    const d = A.Mul(unit, Iinv);
    for (let r = 1; r <= 6; r++) m[r][c] = d[r] || 0;
  });
  return m;
})();
// grade-1 dual coeffs s[1..6] of a pure grade-5 blade (= Mul(C5, Iinv) at grade 1).
export function dualGrade5Coeffs(C5) {
  const s = new Array(7).fill(0);
  for (let c = 0; c < GRADE5_IDX.length; c++) {
    const v = C5[GRADE5_IDX[c]] || 0;
    if (!v) continue;
    for (let r = 1; r <= 6; r++) s[r] += v * DUAL5[r][c];
  }
  return s;
}

// Coefficients from either conic form: a grade-1 vector is already IPNS; a grade-5
// OPNS conic is dualized first (fast linear map for pure grade-5, else full Mul).
export function conicCoeffs(val) {
  const g = gradeFlags(val);
  if (onlyGrade(g, 1)) return coeffsFromGrade1(val);
  if (onlyGrade(g, 5)) return coeffsFromGrade1(dualGrade5Coeffs(val));
