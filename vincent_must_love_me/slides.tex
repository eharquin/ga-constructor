...
\begin{frame}{In one screenshot}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{pga_motor}
  \end{center}
  \vspace{-4pt}
  \footnotesize
  Left: live expression panel. Right: SVG canvas — points, vectors,
  rotors, translators, sandwich transforms — all classified from
  their multivector value. Drag \texttt{P} or move the
  \texttt{t} / \texttt{a} sliders; every dependent updates in
  lock-step.
\end{frame}
...
\begin{frame}{Two algebras today, more tomorrow}
  \begin{columns}[T]
    \column{0.55\textwidth}
      \textbf{VGA(2,0,0) — vectorial 2D}\\[2pt]
      Same expression language, same UI. Vectors, dot, wedge
      (bivector), exponential (rotor), sandwich.

      \medskip
      Adding this algebra was \emph{one file}
      (\texttt{src/algebras/vga200/}) — no changes to parser,
      evaluator, panel, canvas.

      \medskip
      The same recipe will host CGA, 3D~PGA, STA — limited only by
      what ganja.js supports.
...
\begin{frame}{User functions, live}
  \begin{center}
    \includegraphics[width=0.93\textwidth]{pga_fn}
  \end{center}
  \vspace{-6pt}
  \footnotesize
  \texttt{dist(A, B) = |A \& B|} and \texttt{ln(A, B) = A \& B} are
  user-defined (note the green~\textit{\textbf{ƒ}}~badge). The call
  \texttt{d~=~dist(P,~Q)} evaluates to~$6$; \texttt{L~=~ln(P,~Q)}
  draws the join line; \texttt{T~=~[P,~Q,~R]} renders the triangle.
\end{frame}
